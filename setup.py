...
dataexts  = (".py",  ".pyx", ".pxd",  ".c",".h", ".h5", ".bed.gz", ".parquet")
...
ext_module_kmer = declare_cython_extension( "hg19genome.kmers.kmer_reader", use_math=False, use_openmp=False , include_dirs=my_include_dirs )

# this is mainly to allow a manual logical ordering of the declared modules
cython_ext_modules = [ext_module_kmer]
...
setup(
    ...
    url = "https://github.com/kylessmtih/hg19genome",
    ...
    packages = find_namespace_packages(include=["hg19genome", "hg19genome.*"]),
    package_data={'hg19genome': ['*.pxd', '*.pyx', '*.c', '*.h', '*.h5', '*.bed.gz', '*.parquet'],
                  '': ['*.pxd', '*.pyx', '*.c', '*.h', '*.h5', '*.bed.gz', '*.parquet']},
    # Disable zip_safe
    zip_safe = False,
